...
export class Duration {
	...
	constructor(input?: DurationOptions | number) {
		if (!input) this._milliseconds = 0;
			...
			this._milliseconds =
				(milliseconds ?? 0) +
				(seconds ?? 0) * Duration.MS_PER_SECOND +
				(minutes ?? 0) * Duration.MS_PER_MINUTE +
				(hours ?? 0) * Duration.MS_PER_HOUR +
				(days ?? 0) * Duration.MS_PER_DAY +
				(weeks ?? 0) * Duration.MS_PER_WEEK +
				(months ?? 0) * Duration.MS_PER_MONTH +
				(years ?? 0) * Duration.MS_PER_YEAR;
		}
	}
